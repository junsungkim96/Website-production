...
// const SceneDialogCustom = ({ initialData, onClose, onSubmit }) => {
...
//         <h2 style={styles.title}>Scene Luminance</h2>
...
const SceneDialogCustom = ({ initialData, onClose, onSubmit }) => {
  ...
  // 초기 포커스
  useEffect(() => {
    if (inputRefs.current[0][0].current) inputRefs.current[0][0].current.focus();
  }, []);

  // ESC로 취소 시 tableData를 초기화
  const handleCancel = () => {
    setTableData(initialData.map(row => [...row])); // 복사해서 초기화
    if (onClose) onClose(null);
  };

  const handleModify = () => {
    if (onSubmit) onSubmit(tableData.map(row => [...row])); // 복사해서 전달
    if (onClose) onClose(tableData.map(row => [...row]));
  };
...
export default SceneDialogCustom;
